...
test("the app fetches show info and renders it", async () => {
    act(() => {
        mockFetchShow.mockResolvedValueOnce(response);
    });
    const { findByText, queryByText, getByText, debug } = render(<App />);
    waitForElementToBeRemoved(queryByText(/fetching data.../i)).then(async () => {
        debug();
        const showButton = await findByText(/Select a season/i);
