...
test("renders the episodes component from the list of episodes", () => {
    //rendering the component and getting back needed functions
    const { rerender, queryAllByTestId, getByTestId } = render(<Episodes episodes={[]} />);
    
    //checking that the component did render
    const episodesList = getByTestId(/^episodesList$/i);
    expect(episodesList).toBeInTheDOM;
    
    //checking that there are no episodes redered yet when then component first mounts
    const initialEpisodes = queryAllByTestId(/^episode$/i);
    expect(initialEpisodes).toHaveLength(0);

    //checking that episodes appear when the component is rerendered with data
    rerender(<Episodes episodes={episodes}/>);
    const renderedEpisodes = queryAllByTestId(/^episode$/i);
    expect(renderedEpisodes).toHaveLength(3);
});
